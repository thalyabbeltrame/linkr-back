...
export const getFollower = async (req, res) => {
    ...
    try {
        ...
        const { rows: follow } = await followRepository.getFollow(followerId, userId);
        if (!follow[0]) {
            return res.status(200).json({ isFollowing: false })
        }
...
export const follow = async (req, res) => {
    ...
    try {
        ...
        await followRepository.followUser(followerId, userId);
        res.sendStatus(200)
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
...
export const unfollow = async (req, res) => {
    ...
    try {
        ...
        await followRepository.unfollowUser(followerId, userId);
        res.sendStatus(200)
    } catch (error) {
        console.log(error)
        res.status(500).json({ error: error.message });
